...
(function initUnicornGraphEngineBoot(globalObject) {
  ...
  function bootstrapGraphengine() {
    ...
    const treeExport = namespace.treeExport;
    ...
    if (!state || !els || !backend || !metadata || !treeViewport || !treeRender || !treeExport || !ui || !selection || !damage || !reports || !agentUi) {
      throw new Error("Unicorn graphengine boot expected state, DOM, backend, metadata, tree viewport, tree render, tree export, UI, selection, damage, reports, and Agent UI modules to load before boot.");
    }
